...
class PostContract {
  ...
  createPostToken(toAddress, ipfsUri='QmRSj3L3iFf2ix3kE9xJWG7ga3vSEZmYtpg5nY5Nnh5VNo', hash='bofasdfadsfdy', cb) {
    this.Post.methods.createPost(ipfsUri, hash).send({from: this.defaultAccount, gas: '50000'}, (error, transactionHash) => {
      if (error) {
  ...
  _transferOwnership(to='0x60801c0625939a1368c2b753EEBbE50435AC56E6', tokenId) {
    this.Post.methods.transfer(to, tokenId).send({ from: this.defaultAccount, gas: '50000'}).then(receipt => {
